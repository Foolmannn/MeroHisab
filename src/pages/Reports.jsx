      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-6">
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500">Overview for current {range} period</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* MOCK CHART - Representing actual volume */}
          <div className="lg:col-span-8 bg-white dark:bg-slate-900 rounded-xl shadow p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-semibold text-slate-800 dark:text-white">Cash Flow Analysis</h3>
              <button onClick={exportToCSV} className="flex items-center gap-2 text-emerald-500 text-sm cursor-pointer hover:underline">
                <MdFileDownload /> Export CSV
              </button>
            </div>
            <div className="flex items-end gap-2 h-56 border-b border-slate-100 dark:border-slate-800 pb-2">
              {/* This scales based on your real income vs expenses */}
              <div className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
                  <div className="w-12 bg-emerald-400 rounded-t transition-all" style={{ height: `${(stats.income / (stats.income + stats.expenses || 1)) * 100}%` }} />
                  <span className="text-[10px] uppercase font-bold text-slate-400">In</span>
              </div>
              <div className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
                  <div className="w-12 bg-red-400 rounded-t transition-all" style={{ height: `${(stats.expenses / (stats.income + stats.expenses || 1)) * 100}%` }} />
          ...
          <div className="lg:col-span-4 bg-white dark:bg-slate-900 rounded-xl shadow p-6">
            <h3 className="font-semibold mb-4 text-slate-800 dark:text-white">Settlement Status</h3>
            <div className="flex flex-col gap-4">
              <Progress label="Lent Collected" value={Math.round((reportData.filter(t => t.type === 'income' && t.originalType === 'lend').length / reportData.filter(t => t.type === 'lend' || t.originalType === 'lend').length || 0) * 100)} />
              <Progress label="Borrow Paid" value={Math.round((reportData.filter(t => t.type === 'expense' && t.originalType === 'borrow').length / reportData.filter(t => t.type === 'borrow' || t.originalType === 'borrow').length || 0) * 100)} />
            </div>
          </div>

          {/* DATA TABLE */}
          <div className="lg:col-span-12 bg-white dark:bg-slate-900 rounded-xl shadow p-6">
             <h3 className="font-semibold text-slate-800 dark:text-white mb-4">Transaction Details</h3>
            <div className="overflow-x-auto max-h-96 overflow-y-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="dark:text-slate-400">
                  {reportData.sort((a,b) => new Date(b.date) - new Date(a.date)).map((t) => (
                    <Row 
                      key={t.id} 
                      date={t.date} 
...
function Card({ icon, title, value, positive }) {
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl shadow p-5 flex justify-between items-start">
      <div>
        <p className="text-xs text-slate-500 font-bold uppercase tracking-tighter">{title}</p>
...
function Row({ date, desc, amount, status, currency, type }) {
  ...
  return (
    <tr className="border-b dark:border-slate-800 last:border-none">
      ...
      <td className={`py-3 text-right font-bold ${isExpense ? "text-red-500" : "text-emerald-500"}`}>
        {isExpense ? "-" : "+"}{currency}{Number(amount).toLocaleString()}
      </td>
      <td className="py-3 text-right text-[10px] font-bold uppercase">
        <span className={`px-2 py-1 rounded-full ${status === "Settled" || status === "Paid" || status === "Completed" ? "bg-emerald-100 text-emerald-600" : "bg-orange-100 text-orange-600"}`}>
          {status}
        </span>
      </td>
    </tr>
